...
AWS 계정을 추가할 수 있습니다. AWS 계정 등록을 위해서는 AWS 계정의 Access Key와 Secret Key가 필요합니다.

**a\) Access Key와 Secret Key를 발급받기 위해 AWS 서비스 콘솔에서 상단 우측 카테고리에서 'Security Credentials'를 클릭합니다.**![](/assets/2.3.2 AWS 계정 추가.png)
...
**e\) 환경설정 -&gt; 계정 -&gt; 우측 상단 + 모양의 버튼을 클릭 후 페이지에서 프로바이더를 Amazon Web Service 선택 후 이름, 유형,   설명, 앞서 받은 Access Key,Secret Key를 등록 우측 상단 생성 버튼을 클릭합니다.**

**  **![](/assets/2.3.0 계정 추가_AWS.png)
